  ...
  <aside class="hidden md:flex flex-col w-64 bg-white border-r border-gray-100 h-screen sticky top-0">
    ...
    <div class="flex-1 overflow-y-auto px-4 py-2 space-y-1">
      <router-link v-for="item in navItems" :key="item.path" :to="item.path" 
                   class="flex items-center gap-3 px-4 py-2.5 text-gray-500 rounded-full hover:bg-gray-100/50 hover:text-gray-900 transition-all duration-300 text-sm font-medium"
                   active-class="bg-primary-50/70 text-primary-700 font-bold shadow-sm">
    ...
    <div class="p-4 border-t border-gray-100">
      <button @click="$emit('open-create')" class="btn btn-lg w-full mb-4 bg-gradient-to-r from-primary-600 to-primary-500 text-white hover:from-primary-500 hover:to-primary-400 hover:-translate-y-0.5 hover:shadow-lg active:scale-95 transition-all duration-300 rounded-2xl border border-white/20">
        <i class="fas fa-plus mr-2"></i> Create
      </button>
      
      <div v-if="user" class="flex items-center gap-3 px-2 cursor-pointer hover:bg-gray-50 p-3 rounded-2xl transition-all duration-300 hover:shadow-sm border border-transparent hover:border-gray-100" @click="$router.push('/profile')">
        <Avatar :src="user.avatar_url" :name="user.first_name + ' ' + user.last_name" size="sm" />
        <div class="flex-1 min-w-0">
          <p class="text-sm font-bold text-gray-800 truncate">{{ user.first_name }} {{ user.last_name }}</p>
          <p class="text-xs text-gray-500 truncate capitalize font-medium">{{ (user.current_stage || 'corps_member').replace('_', ' ') }}</p>
        </div>
      </div>
    </div>
  </aside>
